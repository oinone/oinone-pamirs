...
public class AdvancedHomePageModelProxyAction {

    @Function.Advanced(type = FunctionTypeEnum.QUERY, managed = true)
    @Function.fun(FunctionConstants.queryPage)
    @Function(openLevel = {FunctionOpenEnum.API})
    public Pagination<AdvancedHomePageModelProxy> queryPage(Pagination<AdvancedHomePageModelProxy> page, IWrapper<AdvancedHomePageModelProxy> queryWrapper) {
        ...
        String rootModuleModule = (String) queryWrapper.getQueryData().get(
                LambdaUtil.fetchFieldName(AdvancedHomePageModelProxy::getRootModuleModule)
        );
        if (StringUtils.isNotEmpty(rootModuleModule)) {
            Set<String> dependencyModules = queryDependencyModules(rootModuleModule);
            ((QueryWrapper<AdvancedHomePageModelProxy>) queryWrapper).in(
                    PStringUtils.fieldName2Column(LambdaUtil.fetchFieldName(AdvancedHomePageModelProxy::getModule)), dependencyModules
            );
        }
        return Models.origin().queryPage(page, queryWrapper);
    }
