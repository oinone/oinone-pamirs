...
@Model.MultiTableInherited(type = "RESOURCE_STREET")
@Model.model(ResourceStreet.MODEL_MODEL)
@Model.Advanced(name = "ResourceStreet", unique = {"code"})
@Model(displayName = "街道", labelFields = "name")
public class ResourceStreet extends BaseResourceModel {

    public static final String MODEL_MODEL = "resource.ResourceStreet";
    ...
    @Field.String
    @Field(required = true, displayName = "街道名称")
    private String name;
    ...
    @Field.one2many
    @Field.Relation(relationFields = {"code", CharacterConstants.SEPARATOR_OCTOTHORPE + ResourceStreet.MODEL_MODEL + CharacterConstants.SEPARATOR_OCTOTHORPE}, referenceFields = {"relationCode", "model"})
    ...
    public static ResourceRegion fetchCurrentRegion(ResourceStreet street) {
        ResourceRegion parentRegion = CommonApiFactory.getApi(ResourceRegionService.class).queryByCode(street.getDistrictCode());
        ...
        return (ResourceRegion) new ResourceRegion()
                ...
                .setType(AddressTypeEnum.Street)
                .setLevel(DefaultResourceConstants.REGION_LEVEL_STREET)
                .setPid(parentRegion.getId())
                .setPCode(parentRegion.getCode())
                .setCountryCode(street.getCountryCode())
                .setHasChildren(Boolean.FALSE)
                .setCode(street.getCode());
    }
}
