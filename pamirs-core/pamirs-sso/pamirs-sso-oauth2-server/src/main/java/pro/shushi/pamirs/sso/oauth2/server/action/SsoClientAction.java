...
public class SsoClientAction {
    ...
    public SsoClient saveSsoOauth2ClientDetails() {
        SsoClient ssoClient = ssoClientService.AutoGenerateClient();
        return ssoClient;
    }

    @Action(displayName = "删除", contextType = ActionContextTypeEnum.SINGLE_AND_BATCH)
    public SsoClient deleteMultipleOrSingleIds(List<SsoClient> ssoClientList) {
        ssoClientService.deleteMultipleOrSingleIds(ssoClientList);
