//package pro.shushi.pamirs.grouping.service.impl;
//
...
//import pro.shushi.pamirs.core.common.tmodel.ConditionWrapper;
...
//import pro.shushi.pamirs.grouping.enumeration.GroupStatisticTypeEnum;
//import pro.shushi.pamirs.grouping.enumeration.GroupingExpEnumerate;
//import pro.shushi.pamirs.grouping.model.*;
//import pro.shushi.pamirs.grouping.service.TableGroupingService;
//import pro.shushi.pamirs.grouping.statistic.GroupStatisticApi;
//import pro.shushi.pamirs.grouping.utils.GroupStatisticUtils;
//import pro.shushi.pamirs.grouping.utils.GroupingUtils;
...
//
//import static pro.shushi.pamirs.grouping.enumeration.GroupStatisticTypeEnum.COUNT;
//import static pro.shushi.pamirs.grouping.enumeration.GroupStatisticTypeEnum.valueOf;
//
///**
...
//
//    @Override
//    public TableGroupingResult fetchGroupPage(Pagination<?> page, TableGroupingWrapper wrapper) {
//        loadGroupBaseInfo(group);
//        return queryGroupInfo(group, page);
//    }
//
//    @Override
//    public TableGroupingResult fetchGroupData(TableGroupingWrapper group) {
//        List<GroupPath> expandGroupPaths = group.getExpandGroupPaths();
...
//        loadGroupBaseInfo(group);
//
//        Pagination<Object> paginationResult = new Pagination<>(1, -1);
//        QueryWrapper<Object> queryWrapper = buildPageQueryWrapper(group);
//        // 构建查询条件查询数据
...
//        }
//
//        TableGroupingResult groupResult = new TableGroupingResult();
...
//        for (GroupPath expandGroupPath : expandGroupPaths) {
...
//
//    @Override
//    public TableGroupingResult fetchGroupStatistic(TableGroupingWrapper group) {
//        List<GroupPath> expandGroupPaths = group.getExpandGroupPaths();
//        if (CollectionUtils.isEmpty(expandGroupPaths)) {
//            throw PamirsException.construct(GroupingExpEnumerate.LAZY_LOAD_PATHS_IS_NULL).errThrow();
//        }
...
//            List<GroupPath> queryExpandGroupPaths = new ArrayList<>(group.getExpandGroupPaths());
...
//     * 加载分组基本信息
//     */
//    private void loadGroupBaseInfo(TableGroupingWrapper group) {
...
//            for (GroupPath expandGroupPath : group.getExpandGroupPaths()) {
//                for (GroupPathNode groupPathNode : expandGroupPath.getNodeList()) {
...
//     * 查询分组信息
//     */
//    private TableGroupingResult queryGroupInfo(final TableGroupingWrapper group, Pagination<?> page) {
//        TableGroupingResult groupResult = new TableGroupingResult();
//
//        // 构建查询条件
//        QueryWrapper<Object> queryWrapper = buildPageQueryWrapper(group);
...
//        Pagination<Object> paginationResult;
//        boolean loadData;
//
...
//        } else {
//            if (needPagination) {
//                Pagination<Object> pagination = addGroupPaginationCondition(group, queryWrapper, page.getCurrentPage(), page.getSize());
...
//            }
//
//            for (GroupingField groupField : group.getFields()) {
...
//
//        // 处理分组值防止前端报错
//        List<GroupingData> groups = groupResult.getGroups();
//        while (CollectionUtils.isNotEmpty(groups)) {
//            List<GroupingData> nextGroups = new ArrayList<>();
//            for (GroupingData groupInfo : groups) {
...
//     * 构建一级分组的分页查询条件
//     */
//    private Pagination<Object> addGroupPaginationCondition(TableGroupingWrapper group, QueryWrapper<Object> queryWrapper, int pageNo, long pageSize) {
//        GroupingField firstGroupField = group.getFields().get(0);
//        ModelFieldConfig firstModelFieldConfig = group.getModelFieldConfig(firstGroupField.getField());
//
//        Pagination<Object> pagination = new Pagination<>(pageNo, pageSize);
...
//        QueryWrapper<Object> groupQueryWrapper = buildPageQueryWrapper(group);
...
//        }
//
//        QueryWrapper<Object> groupNullQueryWrapper = buildPageQueryWrapper(group);
//        Pagination<Object> nullPagination = new Pagination<>(1, 1);
...
//            Pagination<Object> finalPagination = pagination;
...
//     * 添加指定分组路径的查询条件
//     */
//    private void addGroupExpandCondition(TableGroupingWrapper group, QueryWrapper<Object> queryWrapper, List<GroupPath> expandGroupPaths, boolean needSort) {
//        queryWrapper.and(andWrapper -> {
//            for (GroupPath expandGroupPath : expandGroupPaths) {
//                andWrapper.or().and(pathAndWrapper -> {
//                    for (GroupPathNode pathNode : expandGroupPath.getNodeList()) {
...
//     */
//    private QueryWrapper<Object> buildPageQueryWrapper(TableGroupingWrapper group) {
...
//     */
//    private QueryWrapper<Object> parseQueryWrapper(final QueryWrapper<Object> queryWrapper) {
...
//     * 根据分组数据填充分组信息
//     */
//    private void fullGroupInfo(TableGroupingWrapper group, TableGroupingResult groupResult, List<?> dataList, BiConsumer<TableGroupingWrapper, GroupingData> statisticConsumer, boolean loadData) {
//        List<GroupingField> groupFields = group.getFields();
//
//        Map<GroupPath, Map<String, String>> statisticPathMap;
//        if (CollectionUtils.isNotEmpty(group.getExpandGroupPaths())) {
...
//        }
//
//        Map<GroupPath, GroupingData> groupPathMap = new LinkedHashMap<>();
//        Set<GroupPath> firstGroupPathList = new LinkedHashSet<>();
//        Set<GroupPath> lastGroupPathList = new HashSet<>();
//
//        // 加载之前已加载过的分组信息
//        List<GroupingData> beforeGroupFields = groupResult.getGroups();
//        LinkedList<GroupingData> beforeGroupFieldStack = new LinkedList<>();
...
//            GroupingData groupInfo = beforeGroupFieldStack.remove(0);
...
//                GroupingField groupField = groupFields.get(i);
//                GroupingData parentGroupInfo = groupPathMap.get(groupPath);
//
...
//
//                // 判断当前分组是否已存在
//                GroupingData groupInfo = groupPathMap.get(groupPath);
//                boolean isCreateGroup = false;
//                if (groupInfo == null) {
//                    groupInfo = new GroupingData();
...
//            GroupingData lastGroupInfo = groupPathMap.get(groupPath);
...
//        Map<Integer, List<GroupPath>> groupPathMapByNodeNum = new HashMap<>();
//        for (GroupPath groupPath : groupPathMap.keySet()) {
...
//            List<GroupPath> groupPathList = groupPathMapByNodeNum.get(nodeNum);
//            for (GroupPath groupPath : groupPathList) {
//                // 这里的子级groupInfo一定是都填充完成的
//                GroupingData groupInfo = groupPathMap.get(groupPath);
//                groupInfo.storeValue(group);
//                List<GroupingData> childGroups = groupInfo.getGroups();
...
//                    for (GroupingData childGroup : childGroups) {
...
//
//        // 序列化叶子节点数据List
//        for (GroupPath lastGroupPath : lastGroupPathList) {
//            GroupingData lastGroupInfo = groupPathMap.get(lastGroupPath);
...
//     * 统计函数
//     */
//    private BiConsumer<TableGroupingWrapper, GroupingData> statisticFunction() {
...
//                    List<Object> dataList = groupInfo.getDataList();
...
//                        List<Object> notNullDataList = dataList.stream().filter(Objects::nonNull).collect(Collectors.toList());
...
//     * 将空分组值放在当前分组最后面
//     */
//    private void moveGroupNullValueToLast(TableGroupingWrapper group, List<GroupingData> groups) {
//        if (CollectionUtils.isNotEmpty(groups)) {
//            GroupingData groupInfo = groups.get(0);
...
//     * 去数据库查询统计函数
//     */
//    private boolean sqlQueryStatisticDataValues(TableGroupingWrapper group, Map<GroupPath, Map<String, Object>> resultMap, GroupPath groupPath) {
//        Map<String, String> statisticFieldMap = groupPath.getStatisticFieldMap();
...
//        Pagination<Object> pagination = new Pagination<>(1, -1);
//        pagination.setSortable(false);
//        Pagination<Object> paginationResult = Models.origin().queryPage(pagination, parseQueryWrapper(queryWrapper));
//        Object data = paginationResult.getContent().get(0);
//
//        // 获取查询结果
//        statisticFieldMap.forEach((field, statisticType) -> {
//            GroupStatisticTypeEnum statisticTypeEnum = valueOf(statisticType);
...
//     * 内存加载所有数据
//     */
//    private Pagination<Object> loadDataListByMemory(TableGroupingWrapper group, Pagination<Object> page, QueryWrapper<Object> pageQueryWrapper) {
//        for (GroupingField groupField : group.getFields()) {
//            ModelFieldConfig modelFieldConfig = group.getModelFieldConfig(groupField.getField());
...
//        }
//
//        Pagination<Object> pagination = Models.origin().queryPage(new Pagination<>(1, -1), parseQueryWrapper(pageQueryWrapper));
//        if (CollectionUtils.isNotEmpty(pagination.getContent())) {
//            for (GroupingField groupField : group.getFields()) {
//                ModelFieldConfig modelFieldConfig = group.getModelFieldConfig(groupField.getField());
//                if (GroupingUtils.isMemoryGroupField(modelFieldConfig) && !Boolean.TRUE.equals(modelFieldConfig.getStore())) {
//                    List<Object> dataList = Models.origin().listFieldQuery(pagination.getContent(), modelFieldConfig.getField());
...
//    }
//
//    private boolean hasRelationGroupField(TableGroupingWrapper group) {
...
//     * 填充dataList里的引用字段
//     */
//    private void listQueryRelationFields(TableGroupingWrapper group, List<?> dataList) {
