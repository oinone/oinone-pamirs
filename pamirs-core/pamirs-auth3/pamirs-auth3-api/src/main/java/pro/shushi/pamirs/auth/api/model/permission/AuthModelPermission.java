 ...
 * 模型权限项
 *
 * @author Adamancy Zhang at 15:30 on 2024-01-04
 */
@Base
@Model.model(AuthModelPermission.MODEL_MODEL)
@Model.Advanced(unique = {"code,model"})
@Model(displayName = "模型权限项")
public class AuthModelPermission extends IdModel implements AuthPermission {

    private static final long serialVersionUID = 2796785137824074743L;

    public static final String MODEL_MODEL = "auth.AuthModelPermission";
    ...
    public String refreshCode() {
        String code = generatorCode(getModel(), getPath());
        setCode(code);
        return code;
    }
    ...
    @Override
    public String sign() {
        return StringHelper.join(CharacterConstants.SEPARATOR_OCTOTHORPE, getModel(), getCode());
    }

    public static String generatorCode(String model) {
        return generatorCode(model, null);
    }

    public static String generatorCode(String model, String path) {
    ...
    public static <T extends AuthModelPermission> T transfer(AuthModelPermission origin, T target) {
