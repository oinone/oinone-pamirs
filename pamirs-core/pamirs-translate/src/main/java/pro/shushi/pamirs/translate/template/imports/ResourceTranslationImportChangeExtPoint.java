...
/**
 * @author: xuxin
 * @createTime: 2024/05/07 16:52
 */
@Slf4j
@Component
@Ext(ExcelImportTask.class)
public class ResourceTranslationImportChangeExtPoint extends AbstractExcelImportDataExtPointImpl<ResourceTranslationItem> {
    @Autowired
    private ResourceTranslationImportTask resourceTranslationImportTask;

    @Override
    @ExtPoint.Implement(expression = "importContext.definitionContext.name==\"" + ChangeTranslateTemplate.TEMPLATE_NAME + "\" &&  importContext.definitionContext.model==\"" + ResourceTranslation.MODEL_MODEL + "\"")
    ...
    private void calcExcel(List<ResourceTranslationItem> excelItemList, ExcelImportTask importTask) {
        ...
        for (List<ResourceTranslationItem> subMap : itemMetaSplitList) {
            ...
            ResourceTranslationImportChangeExtPoint.TranslateItemSaveRunnable translateItemSaveRunnable = new ResourceTranslationImportChangeExtPoint.TranslateItemSaveRunnable(subMap, threadSignal);
