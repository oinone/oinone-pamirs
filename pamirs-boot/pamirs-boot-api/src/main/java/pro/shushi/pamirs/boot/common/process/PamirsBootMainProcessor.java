...
import pro.shushi.pamirs.framework.configure.annotation.core.FuseModuleResolver;
...
public class PamirsBootMainProcessor implements PamirsBootMainProcessApi {
    ...
    public Boolean installOrLoad(final ComputeContext context, final Set<String> modules, final Set<String> excludeModules,
        ...
        Spider.getDefaultExtension(BootModuleLifecycleAroundApi.class).run(command, context, setupModuleMap, bootModuleSet, (_command, _context, _setupModuleMap, _bootModuleSet) -> {
            ...
            // 从db查询需要安装的模块(无代码), 需要租户信息
            Map<String, ModuleDefinition> dataModules = bootModulesApi.dataModules(_command);
            _setupModuleMap.putAll(dataModules);

            if (Boolean.TRUE.equals(bootModulesApi.initNoCodeModules())) {
                Set<String> noCodeModules = bootModulesApi.noCodeModules();
                if (CollectionUtils.isEmpty(noCodeModules)) {
                    // 默认全部安装
                    _bootModuleSet.addAll(dataModules.keySet());
                } else {
                    _bootModuleSet.addAll(noCodeModules);
                }
            }
    ...
    public void fetchFuseModel(Map<String, ModuleDefinition> setupModuleMap) {
        FuseModuleResolver fuseModuleResolver = BeanDefinitionUtils.getBean(FuseModuleResolver.class);
        Map<String, String> fuseModuleMap = fuseModuleResolver.resolve();
        for (Map.Entry<String, String> entry : fuseModuleMap.entrySet()) {
            String fuseModule = entry.getKey();
            ModuleDefinition fuseModuleD = setupModuleMap.get(fuseModule);
            String[] packagePrefix = fuseModuleD.getPackagePrefix();
            Set<Class<?>> classSet = ClassUtils.getClassesByPacks(packagePrefix);
            for (Class<?> klass : classSet) {
                if (null == klass) {
                    continue;
                }
                Model.Fuse fuseAnnotation = AnnotationUtils.getAnnotation(klass, Model.Fuse.class);
                if (null == fuseAnnotation) {
                    continue;
                }
                Model.model mmAnnotation = AnnotationUtils.getAnnotation(klass, Model.model.class);
                if (null == mmAnnotation) {
                    continue;
                }
                String mm = mmAnnotation.value();
                FuseMeta.put(mm, entry.getValue(), klass.getCanonicalName());
            }
        }
    }
