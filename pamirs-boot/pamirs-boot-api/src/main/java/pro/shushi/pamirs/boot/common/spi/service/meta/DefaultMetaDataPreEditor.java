...
@Component
@SPI.Service
public class DefaultMetaDataPreEditor implements MetaDataPreEditApi {

    @Override
    public void edit(AppLifecycleCommand command, Map<String/*module*/, Meta> metaMap) {
        for (MetaDataPreEditor editor : BeanDefinitionUtils.getBeansOfTypeByOrdered(MetaDataPreEditor.class)) {
            long start = System.currentTimeMillis();
            editor.edit(command, metaMap);
            log.info("{} meta data pre editor cost time: {}ms", editor.getClass().getName(), System.currentTimeMillis() - start);
        }
    }
