...
public class ScheduleUtil {
    ...
    private static String OWN_SIGN_SPLIT = "$";
    ...
    public static String getTaskTypeByBaseAndOwnSign(String baseType, String ownSign) {
        ...
        return baseType + OWN_SIGN_SPLIT + ownSign;
    }

    public static String splitBaseTaskTypeFromTaskType(String taskType) {
        int index = taskType.indexOf(OWN_SIGN_SPLIT);
        if (index >= 0) {
            return taskType.substring(0, index);
        } else {
            return taskType;
        }
    }
    ...
    public static String splitOwnsignFromTaskType(String taskType) {
        int index = taskType.indexOf(OWN_SIGN_SPLIT);
        if (index >= 0) {
            return taskType.substring(index + 1);
        } else {
