...
import pro.shushi.pamirs.framework.connectors.data.sql.api.Compare;
import pro.shushi.pamirs.framework.connectors.data.sql.api.Func;
import pro.shushi.pamirs.framework.connectors.data.sql.api.Join;
import pro.shushi.pamirs.framework.connectors.data.sql.api.Nested;
import pro.shushi.pamirs.framework.connectors.data.sql.contants.SqlLike;
import pro.shushi.pamirs.framework.connectors.data.sql.contants.WrapperConstants;
import pro.shushi.pamirs.framework.connectors.data.sql.segments.MergeSegments;
import pro.shushi.pamirs.framework.connectors.data.sql.utils.SqlUtils;
import pro.shushi.pamirs.framework.connectors.data.sql.utils.StringEscape;
...
import static pro.shushi.pamirs.framework.connectors.data.sql.contants.WrapperKeyword.*;
import static pro.shushi.pamirs.framework.connectors.data.sql.enmu.SqlExpEnumerate.*;
...
public abstract class AbstractWrapper<T, R, Children extends AbstractWrapper<T, R, Children>> extends Wrapper<T>
    ...
    protected Children setEntity(DataMap entity) {
    ...
    public String getModel() {
        ...
        if (StringUtils.isBlank(model)) {
            T t = getEntity();
            if (null == t) {
                log.error(BASE_MODEL_CONFIG_ERROR.msg(), PamirsException.construct(BASE_MODEL_CONFIG_ERROR));
            } else {
                model = Models.api().getModel(t);
            }
        }
        return model;
    }
    ...
    public Children from(String model) {
        this.setModel(model);
        return typedThis;
    }

    public Children from(Class<?> modelClass) {
        this.setModel(Models.api().getModel(modelClass));
        return typedThis;
    }
    ...
    public <V> Children allEq(boolean condition, Map<R, V> params, boolean null2IsNull) {
        if (condition && MapUtils.isNotEmpty(params)) {
            params.forEach((k, v) -> {
                if (PStringUtils.checkValNotNull(v)) {
                    eq(k, v);
                } else {
                    if (null2IsNull) {
                        isNull(k);
    ...
    @Override
    public Children eq(boolean condition, Collection<R> column, List<?> coll) {
        return doIt(condition, columnsToString(column), EQ, insExpression(coll));
    }
    ...
    public Children last(boolean condition, String lastSql) {
        if (condition) {
            this.lastSql.setStringValue(CharacterConstants.SEPARATOR_BLANK + lastSql);
        }
        return typedThis;
    }
    ...
    @Override
    public Children in(boolean condition, Collection<R> column, List<?>... coll) {
        return doIt(condition, columnsToString(column), IN, insExpression(coll));
    }
    ...
    @Override
    public Children orderByNe(boolean condition, R column, String param) {
        if (null == column || StringUtils.isBlank(param)) {
            return typedThis;
        }
        ISqlSegment seg = () -> columnToString(column).getSqlSegment() + param;
        doIt(condition, ORDER_BY, seg);
        return typedThis;
    }
    ...
    protected final String formatSqlIfNeed(boolean need, String sqlStr, Object... params) {
        ...
        if (ArrayUtils.isNotEmpty(params)) {
            for (int i = 0; i < params.length; ++i) {
                String genParamName = WrapperConstants.WRAPPER_PARAM + paramNameSeq.incrementAndGet();
                sqlStr = sqlStr.replace(String.format("{%s}", i),
                        String.format(WrapperConstants.WRAPPER_PARAM_FORMAT, WrapperConstants.WRAPPER, genParamName));
     ...
     * @param values 集合
     */
    protected ISqlSegment insExpression(List<?>[] values) {
        if (1 == values.length) {
            return () -> values[0].stream().map(v -> formatSql("{0}", v))
                    .collect(joining(CharacterConstants.SEPARATOR_COMMA, CharacterConstants.LEFT_BRACKET, CharacterConstants.RIGHT_BRACKET));
        }
        List<String> expressions = new ArrayList<>();
        for (int i = 0; i < values[0].size(); i++) {
            int j = i;
            expressions.add(Arrays.stream(values).map(v -> formatSql("{0}", v.get(j)))
                    .collect(joining(CharacterConstants.SEPARATOR_COMMA, CharacterConstants.LEFT_BRACKET, CharacterConstants.RIGHT_BRACKET)));
        }
    ...
    protected ISqlSegment insExpression(List<?> values) {
        return () -> values.stream().map(v -> formatSql("{0}", v))
                .collect(joining(CharacterConstants.SEPARATOR_COMMA, CharacterConstants.LEFT_BRACKET, CharacterConstants.RIGHT_BRACKET));
    }
     ...
     * 获取 columnName
     */
    protected ISqlSegment columnToString(R column) {
        if (null == column) {
        ...
        throw PamirsException.construct(BASE_COLUMN_TYPE_ERROR).errThrow();
    }

    protected ISqlSegment columnsToString(Collection<R> columns) {
        return () -> {
            if (1 == columns.size()) {
                R r = columns.iterator().next();
                if (r instanceof String) {
                    return (String) r;
                }
            } else {
                return columns.stream().map(v -> (String) v)
                        .collect(joining(CharacterConstants.SEPARATOR_COMMA, CharacterConstants.LEFT_BRACKET, CharacterConstants.RIGHT_BRACKET));
            }
            throw PamirsException.construct(BASE_COLUMN_TYPE2_ERROR).errThrow();
        };
    }
     ...
     * @param columns 多字段
     */
    protected ISqlSegment columnsToString(R... columns) {
        return () -> Arrays.stream(columns).map(this::columnToString).map(ISqlSegment::getSqlSegment).collect(joining(CharacterConstants.SEPARATOR_COMMA));
    }

    @Override
    @SuppressWarnings("all")
    public Children clone() {
        return ObjectUtils.clone(typedThis);
    }

    public DataMap genericCheck(DataMap entity) {
        if (null == entity) {
            return null;
        }
        if (null == entity.getModel()) {
            throw PamirsException.construct(BASE_MODEL_MODEL_IS_NOT_EXISTS_ERROR).errThrow();
        }
        return entity;
    }
