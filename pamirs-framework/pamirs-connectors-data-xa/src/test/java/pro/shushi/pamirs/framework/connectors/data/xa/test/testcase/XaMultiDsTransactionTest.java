package pro.shushi.pamirs.framework.connectors.data.xa.test.testcase;
...
import pro.shushi.pamirs.framework.connectors.data.xa.test.mock.model.TestFixedDsModel;
import pro.shushi.pamirs.framework.connectors.data.xa.test.mock.model.TestMultiDsModel;
import pro.shushi.pamirs.framework.connectors.data.xa.test.mock.model.TestXaDsModel;
import pro.shushi.pamirs.framework.connectors.data.xa.test.mock.service.TestTransactionService;
 ...
 */
@Slf4j
@DisplayName("XA多数据源分布式事务测试")
public class XaMultiDsTransactionTest extends AbstractBaseTest {
    ...
    public void testInsertWithTransactionInnerNewService(){
        // 执行测试用例
        try{
            DataMap one = one(TestXaDsModel.modelModel);
            ...
            // 结果断言
            Map<String, Object> result = genericMapper.selectOne(Pops.<DataMap>query().from(TestXaDsModel.modelModel).eq("`module`", "yihui"));
            AssertionErrors.assertNotEquals("插入并查询单条记录失败", null, result);
    ...
    public void testInsertWithTransactionInnerJoinServiceRollback(){
        ...
        try{
            DataMap one = one(TestFixedDsModel.modelModel);
            DataMap ds0 = one(TestXaDsModel.modelModel);
            ds0.setValue("module", "yihui1");
            ...
            result = genericMapper.selectOne(Pops.<DataMap>query().from(TestXaDsModel.modelModel).eq("`module`", "yihui1"));
            AssertionErrors.assertEquals("插入并查询单条记录失败", null, result);
        }
    }
