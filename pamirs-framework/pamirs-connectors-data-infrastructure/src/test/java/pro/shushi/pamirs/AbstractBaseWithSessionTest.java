...
abstract
public class AbstractBaseWithSessionTest {
...
//        ddl(ddl);
    ...
    void init() throws SQLException {
        ...
        LogicSchemaService.get().initSystemSchema(new String[]{
                ...
                DatabaseSchema.MODEL_MODEL,
        }, true);

        constructSessionAndTable();
    ...
    private void constructSessionAndTable() {
        // 准备数据
        String baseModule = "base";
        MetaData baseMetaData = fetchMetaData(baseModule);

        String test1Module = "test1";
        MetaData test1MetaData = fetchMetaData(test1Module);

        String test2Module = "test2";
        MetaData test2MetaData = fetchMetaData(test2Module);

        Sessions.fillSession(Lists.newArrayList(baseMetaData, test1MetaData, test2MetaData));


        // 按模块处理业务表
        Meta baseMeta = fetchMeta(baseModule);

        Map<String, MetaData> dependencyMetaData1 = new ConcurrentHashMap<>();
        dependencyMetaData1.put(baseModule, baseMetaData);
        Meta test1Meta = fetchMeta(test1Module, dependencyMetaData1);

        Map<String, MetaData> dependencyMetaData2 = new ConcurrentHashMap<>();
        dependencyMetaData2.put(baseModule, baseMetaData);
        ...
        LogicSchemaService.get().buildTable(metaList);
    }
