...
/**
 * 通用查询 SQl Statement 生成方法
 *
 * @author paidaxing
 * @version 1.0.0
 * @date 2024/03/15 10:26:29
 */
@Data
public class GaussSelectMethod extends AbstractGaussMethod implements SelectMethod {
    ...
    public GaussSelectMethod(ModelConfig modelConfig) {
        super(modelConfig);
    }
    ...
    public String logicSegment() {
        ...
        if (logicDelete) {
            script = PostgreSQLScriptTemplate.LOGIC_DELETE;
            script += NEWLINE;
        }
        ...
        if (useOptimisticLocker && StringUtils.isNotBlank(optimisticLockerField)) {
            script += "AND " + String.format(PostgreSQLScriptTemplate.OPTIMISTIC_LOCKER,
