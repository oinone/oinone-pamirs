 ...
 */
@Data
public class SqlServerSelectMethod extends AbstractSqlServerMethod implements SelectMethod {
    ...
    public SqlServerSelectMethod(ModelConfig modelConfig) {
        super(modelConfig);
    }
    ...
    public String logicSegment() {
        ...
        if (logicDelete) {
            script = SqlServerScriptTemplate.LOGIC_DELETE;
            script += NEWLINE;
        }
        ...
        if (useOptimisticLocker && StringUtils.isNotBlank(optimisticLockerField)) {
            script += "AND " + String.format(SqlServerScriptTemplate.OPTIMISTIC_LOCKER,
    ...
    public String sqlComplexes(List<ModelFieldConfig> modelFieldConfigList) {
        List<String> columns = modelFieldConfigList.stream()
                .map(i -> ModelFieldConfigWrapper.wrap(i).getSqlSelect()).collect(Collectors.toList());
        List<String> values = modelFieldConfigList.stream()
                .map(i -> SqlScriptUtils.safeParam(ITEM_DOT + i.getLname())).collect(Collectors.toList());

        String sqlScript = logicSegment();
        sqlScript += convertForeach(columns, values, CONDITION_COLLECTION, INDEX, ITEM, "OR");
        sqlScript = SqlScriptUtils.convertWhere(sqlScript) + NEWLINE;
        return sqlScript;
    }
    ...
    //TODO 2.0
    public static String convertForeach(final List<String> columns, final List<String> values, final String collection, final String index,
        ...
        sb.append(" ").append("open='('").append(" ").append("close=')'").append(" ");

        //TODO
        StringBuilder sqlScript = new StringBuilder("(");
        for (int i = 0; i < columns.size(); i++) {
            if (i == 0) {
                sqlScript.append(columns.get(i) + "=" + values.get(i));
            } else {
                sqlScript.append(" ").append(AND).append(" ").append(columns.get(i) + "=" + values.get(i));
            }
        }
        //TODO
        sqlScript.append(")");
        return sb.append(RIGHT_CHEV).append(NEWLINE).append(sqlScript.toString()).append(NEWLINE).append("</foreach>").toString();
    }
    ...
    public String chooseSelect(boolean onlyColumn) {
        ...
        String fieldsSqlSelect = getModelConfig().getModelFieldConfigList().stream()
    ...
    public String getAllSqlWhere(boolean withPk, final String prefix) {
        ...
        return modelConfig.getModelFieldConfigList().stream()
                .filter(ModelFieldConfig::getStore)
    ...
    public String getUniqueKeySqlWhere(final String prefix) {
        ...
        return getModelConfig().getModelFieldConfigList().stream()
