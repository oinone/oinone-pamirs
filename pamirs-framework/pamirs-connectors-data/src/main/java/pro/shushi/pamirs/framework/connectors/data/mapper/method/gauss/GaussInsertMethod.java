...
import pro.shushi.pamirs.framework.connectors.data.mapper.method.postgresql.constants.PostgreSQLScriptTemplate;
...
/**
 * 通用插入SQL statement 生成方法
 *
 * @author paidaxing
 * @version 1.0.0
 * @date 2024/03/15 08:48:08
 */
@Data
public class GaussInsertMethod extends AbstractGaussMethod implements InsertMethod {
    ...
    public GaussInsertMethod(ModelConfig modelConfig) {
        super(modelConfig);
    }
    ...
    public String onDuplicateKeyUpdate() {
        ...
        String sqlScript = modelConfig.getModelFieldConfigList().stream()
        ...
        sqlScript = String.format(PostgreSQLScriptTemplate.ON_DUPLICATE_KEY_UPDATE, sqlScript);
    ...
    public String getAllInsertSqlColumnMaybeIf(ModelConfig modelConfig, final String prefix, final boolean batch) {
        ...
        String script = modelConfig.getModelFieldConfigList().stream()
                .filter(modelFieldConfig -> modelFieldConfig.getStore() && !isAutoIncrementModelField(modelConfig, modelFieldConfig))
    ...
    public String getInsertOrUpdateSqlColumn(ModelFieldConfig modelFieldConfig) {
        ...
        return SQLMethodUtils.getInsertOrUpdateSqlColumn(column);
    }
