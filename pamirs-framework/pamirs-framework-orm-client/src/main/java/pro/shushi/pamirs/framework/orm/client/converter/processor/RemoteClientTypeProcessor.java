...
import pro.shushi.pamirs.framework.orm.client.converter.entity.RemoteClientRelationConverter;
 ...
 */
@Component
public class RemoteClientTypeProcessor implements FieldConverterApi {
    ...
    @Resource
    private RemoteClientRelationConverter remoteClientRelationConverter;
    ...
    public void in(FieldComputeContext context, ModelFieldConfig fieldConfig, Map<String, Object> origin) {
        ...
        TtypeEnum.switches(ttype, caseValue(),
                ...
                cases(O2M, M2O, M2M, O2O).to(() -> remoteClientRelationConverter.in(totalContext, fieldConfig, origin))// 关系类型处理
        );
    }
    ...
    public void out(FieldComputeContext context, ModelFieldConfig fieldConfig, Map<String, Object> origin) {
        ...
        TtypeEnum.switches(ttype, caseValue(),
                ...
                cases(O2M, M2O, M2M, O2O).to(() -> remoteClientRelationConverter.out(fieldConfig, origin))// 关系类型处理
        );
    }
