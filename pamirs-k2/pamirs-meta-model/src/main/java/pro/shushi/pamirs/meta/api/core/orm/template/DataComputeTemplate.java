...
@Component
public class DataComputeTemplate {
    ...
    public static DataComputeTemplate getInstance() {
        if (null == INSTANCE) {
            synchronized (DataComputeTemplate.class) {
                if (null == INSTANCE) {
                    DataComputeTemplate.INSTANCE = new DataComputeTemplate();
                }
            }
        }
    ...
    public <T, R> R compute(String model, T origin, ModelIteratorComputeApi cycleComputeApi,
                            ModelBeforeComputeApi modelBeforeComputeProcessor, ModelAfterComputeApi modelAfterComputeProcessor,
        ...
        return compute(null, model, origin,
                ...
                (context, oModel, oOrigin) -> modelAfterComputeProcessor.after(model, oOrigin),
        ...
        try {
            return ((OrmComputer<T, R>) ormComputer).compute(model, origin,
                    (oModel, oOrigin) -> {// model & map
                        ...
                        if (!CollectionUtils.isEmpty(modelFieldConfigList)) {
                            if (null != fieldComputeProcessors) {
                                ...
                                for (ModelFieldConfig fieldConfig : modelFieldConfigList) {
                                    ...
                                    context.setOp(null);
                                    try {
                                        for (FieldComputeApi fieldComputeProcessor : fieldComputeProcessors) {
                                            if (FieldComputeOp.skipNextProcessor.equals(context.getOp())) {
                                                continue;
                                            }
                                            fieldComputeProcessor.run(context, fieldConfig, dMap);
                        ...
                        oOrigin = (R) modelAfterComputeProcessor.after(totalContext, oModel, oOrigin);
                        return oOrigin;
                    },
                    (oModel, oMap) -> {// list
                        List list = ((List) oMap);
