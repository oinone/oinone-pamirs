...
import pro.shushi.pamirs.ux.grouping.entity.BasicTableGroupingFieldQuery;
...
import pro.shushi.pamirs.ux.grouping.statistic.StatisticApiFactory;
...
public class TableGroupingStatisticUniversalQuery<T> implements TableGroupingStatisticQueryApi<T> {
    ...
    private String fetchGroupingStatisticData(TableGroupingModel model, List<TableGroupingFieldQuery> queryList, QueryWrapper<T> queryWrapper) {
        ...
        if (api == null) {
            throw PamirsException.construct(UxCommonExpEnumerate.STATISTIC_API_NOT_FOUND, statisticMethod).errThrow();
        }
        ...
        if (needQueryRelationFields.isEmpty() && memoryQueryList.isEmpty()) {
            QueryHelper.queryDataListByQueryPage(model.getModel(), queryWrapper, api::compute);
        } else if (memoryQueryList.isEmpty()) {
            QueryHelper.queryDataListByQueryPage(model.getModel(), queryWrapper, (list) -> {
