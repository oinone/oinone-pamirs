 ...
 * @author deng
 */
public class PamirsOnFaaSProcessor extends AbstractProcessor {
    ...
    // 因为是单线程，所以不用考虑并发
    private java.util.List<FunctionDefine> functions = new ArrayList<>();

    private java.util.List<String> exludeMethods = new java.util.ArrayList(){{
        add("<init>");
        add("equals");
        add("canEqual");
        add("hashCode");
                ...
                public void visitClassDef(JCTree.JCClassDecl jcClassDecl) {
                    ...
                    for (JCTree tree : jcClassDecl.defs) {
                        if (tree.getKind().equals(Tree.Kind.METHOD)) {
                            ...
                            if(exludeMethods.contains(funName)){
                                continue;
                            }
